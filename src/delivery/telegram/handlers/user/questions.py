...
from delivery.telegram.keyboards.moderator.questions_answer import build_question_answer_kb
...
from delivery.telegram.keyboards.callback_data import QUESTIONS_PER_PAGE, ListedQuestionAction, ListedQuestionCallbackData, QuestionPaginatorCallbackData
...
@r.callback_query(ListedQuestionCallbackData.filter(F.action == ListedQuestionAction.SHOW))
@inject
async def on_show_question(call: CallbackQuery, callback_data: ListedQuestionCallbackData, service: FromDishka[QuestionService]):
    ...
    await call.message.edit_text(
        text,
        reply_markup=build_back_kb("list_questions")
    )

@r.callback_query(QuestionPaginatorCallbackData.filter())
@inject
async def on_page_change(call: CallbackQuery, callback_data: QuestionPaginatorCallbackData, service: FromDishka[QuestionService]):
...
@inject
async def on_question_received(message: Message, bot: Bot, state: FSMContext, settings: FromDishka[TelegramSettings]):
    ...
    await message.answer("Ваш вопрос получен! Модератор ответит на него в ближайшее время.")
    await state.clear()