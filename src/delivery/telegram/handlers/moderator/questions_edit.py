...
@r.callback_query(ListedQuestionCallbackData.filter(F.action == ListedQuestionAction.EDIT))
@inject
async def on_show(
    call: CallbackQuery,
...
@r.callback_query(QuestionEditCallbackData.filter(F.action != QuestionEditAction.DELETE))
async def on_start_edit(
    ...
    await call.message.edit_text(
        ...
        reply_markup=build_back_kb("stop_edit")
    )
...
async def on_delete(
    ...
    await render_delete_confirm(call, callback_data.question_id, back_to_detail_callback)


@r.callback_query(QuestionEditAgreeCallbackData.filter(F.action == QuestionEditAction.DELETE))
@inject
